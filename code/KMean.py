...
def findClosestCentroids(X, centroids):
    ...
    for i in range(X.shape[0]):
        # Calculate the Euclidean distance between this example and each centroid in the centroids array
        distances = np.linalg.norm(X[i] - centroids, axis=1)
        
        # Find the index of the centroid with the smallest distance relative to the current example
        closest_centroid_index = np.argmin(distances)
        
        # Assign the index of the closest centroid to the current example in the idx array
        idx[i] = closest_centroid_index
    
    return idx

# Load your data (replace 'data.npy' with your actual file path)
...
# Find the closest centroids for each example in X
...
def kMeansInitCentroids(X, K):
    ...
    #On génère un ensemble de K indices distincts sélectionnés aléatoirement à partir de (0, m-1). 
    #On utilise np.random.choice avec replace=False pour garantir que les indices sont uniques.
    indices = np.random.choice(m, K, replace=False)
    
    #On eextrait les exemples de la matrice X aux indices sélectionnés aléatoirement 
    #pour former les centroids initiaux. Chaque ligne de centroids représente un centroid.
    centroids = X[indices, :]
